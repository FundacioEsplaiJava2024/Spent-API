...
public class EventController {
    ...
    public ResponseEntity<?> joinEvent(@PathVariable Integer id) {
        ...
        if (event.getEventParticipants().size() >= event.getNumParticipants()) return ResponseEntity.status(HttpStatus.OK).body("Event already filled");
        
        if (event.getEventParticipants().contains(user)) return ResponseEntity.status(HttpStatus.OK).body("You already joined this event");

        else return ResponseEntity.status(HttpStatus.OK).body(eventService.joinEvent(event, user));        
    }
